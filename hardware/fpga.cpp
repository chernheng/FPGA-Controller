...
//      filter x,y,z ; random number(get val from timer? or server)
//      input,output from host

//GAME STATES
int lobby(){     //TODO:idk what we want here, button press? diplay id?
	alt_printf("lobby");
...
int z_axis_task(){  //TODO: change to z-axis //could change/remove?? delay
    ...
    while(shake_count>0){
    	...
    	while(x_read>30){
    		alt_up_accelerometer_spi_read_x_axis(acc_dev, & x_read);
    		alt_printf("tilted left: %x\n", x_read);
    	}
    	x_read=-40;
    	for(int k=0; k<1000000;k++){;}
    	while(x_read<-30){
    		alt_up_accelerometer_spi_read_x_axis(acc_dev, & x_read);
    		alt_printf("tilted right: %x\n", x_read);
    	}
    	shake_count--;
    	alt_printf("shake %x\n", shake_count);
    }
...
int main(){
    ...
    while(1){
       ...
       // alt_printf  ("gimme an input, 7 to exit") ;
        ...
        //TODO: send to host instead of print  //NOTE: we dont need z-axis value
