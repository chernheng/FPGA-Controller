...
const alt_u16 FIR_SHIFT = 20;	// number of bits to shift output FIR data
...
alt_u8 accel_samplerate = ACCEL_400HZ;
...
int z_axis_task(){
    ...
    // use fifo_data[2] to read filtered (HPF) z-axis
    int shake_count = 10;
    int state = 1;
    while(1){

    	if(state == 1 && fifo_data[2] < 30 ){state=0; shake_count--;}
    	if(state == -1 && fifo_data[2] > 30 ){state=1; shake_count--;}
    	if(state == 0 ) { state=-1;}
