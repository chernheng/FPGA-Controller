...
int main()
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            send(clients.socket_descriptor[0], &sendchar, sizeof(sendchar), 0); // This will be substituted to be the STATE for the player
            ...
            int iBytesReceived;
            //error returns -1
            if ((iBytesReceived = read(clients.socket_descriptor[0], recv_data, MAX_COUNT_BYTES)) < 0)
            {
                printf("Error in reading received bytes\n");
            }
            printf("Read bytes received: %d.\n", iBytesReceived);

            //recv_data[iBytesReceived]='\0';
            //check if packet is of fpga packet
            int pkt_header_type;
            pkt_header_type = process_packet_header(recv_data);
            if (pkt_header_type == 1)
            {
                //from fpga
                int process = process_fpga_coord(recv_data, iBytesReceived);
            }
